...
{ 
  ...
  fDist2Cmd = new G4UIcmdWithADoubleAndUnit("/testem/det/setDist2",this);
  fDist2Cmd->SetGuidance("Set distance between each pixel module and the DUT");
  fDist2Cmd->SetParameterName("Dist2",false);
  fDist2Cmd->SetRange("Dist2>0.");
  fDist2Cmd->SetUnitCategory("Length");
  fDist2Cmd->AvailableForStates(G4State_PreInit,G4State_Idle);
...
{ 
  ...
  if( command == fDist2Cmd )
   { fDetector->SetDist2(fDist2Cmd->GetNewDoubleValue(newValue));}
